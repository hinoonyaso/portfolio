...
**핵심 기술 (Why)**
- **Nav2(TEB)**: 120cm 복도/0.4m/s 제한에서 jitter가 커져 cost 가중치 조정
- **TF2**: 센서 프레임 드리프트를 줄이기 위해 TF 트리 정합 유지
...
**실무 연결**: 현장 이동체에서 재전송/타임아웃 정책으로 통신 오류를 줄인 적용 장면을 재현.
...
**실무 연결**: Edge 컴퓨팅 기반 시스템 통합과 액추에이터 제어 안정화에 적용.
